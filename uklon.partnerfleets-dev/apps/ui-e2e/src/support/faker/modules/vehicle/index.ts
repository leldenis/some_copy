import { PhotosDto, PictureUrlDto } from '@data-access';

import { ModuleBase } from '../_internal';

type VehiclePicture = Pick<
  PhotosDto,
  | 'vehicle_interior_front_photo'
  | 'vehicle_interior_back_photo'
  | 'driver_car_right_sight_photo'
  | 'driver_vehicle_registration_reverse_copy'
  | 'driver_car_front_photo'
  | 'driver_insurance_front_copy'
  | 'driver_car_sight_photo'
  | 'driver_vehicle_registration_front_copy'
  | 'driver_car_back_photo'
>;

export type BuildProps = Partial<{
  vehicleInteriorFront: Partial<PictureUrlDto>;
  vehicleInteriorBack: Partial<PictureUrlDto>;
  driverCarRightSight: Partial<PictureUrlDto>;
  driverVehicleRegistrationReverse: Partial<PictureUrlDto>;
  driverCarFront: Partial<PictureUrlDto>;
  driverInsuranceFront: Partial<PictureUrlDto>;
  driverCarSight: Partial<PictureUrlDto>;
  driverVehicleRegistrationFront: Partial<PictureUrlDto>;
  driverCarBack: Partial<PictureUrlDto>;
}>;

const placeholder =
  'data:image/png;base64,';

export class VehicleImagesModule extends ModuleBase<VehiclePicture, BuildProps> {
  public buildDto(props?: BuildProps): VehiclePicture {
    return {
      vehicle_interior_front_photo: this.buildPicture(props?.vehicleInteriorFront),
      vehicle_interior_back_photo: this.buildPicture(props?.vehicleInteriorBack),
      driver_car_right_sight_photo: this.buildPicture(props?.driverCarRightSight),
      driver_vehicle_registration_reverse_copy: this.buildPicture(props?.driverVehicleRegistrationReverse),
      driver_car_front_photo: this.buildPicture(props?.driverCarFront),
      driver_insurance_front_copy: this.buildPicture(props?.driverInsuranceFront),
      driver_car_sight_photo: this.buildPicture(props?.driverCarSight),
      driver_vehicle_registration_front_copy: this.buildPicture(props?.driverVehicleRegistrationFront),
      driver_car_back_photo: this.buildPicture(props?.driverCarBack),
    };
  }

  private buildPicture(picture?: Partial<PictureUrlDto>): PictureUrlDto {
    return {
      url: placeholder,
      fallback_url: placeholder,
      uploaded_at: 1_667_286_956,
      ...picture,
    };
  }
}
