...
describe.skip('Forgot password page', () => {
  ...
  describe('Confirmation code', () => {
    ...
    it.skip('[PF-64] Should disable "Submit code" button if phone number is invalid', () => {
      cy.getBySel('phone-number-control').type('929232');
      cy.getBySel('receive-recovery-code-button').should('be.disabled');
    });

    it.skip('[PF-65] Should enable "Submit code" button if phone number is valid', () => {
      cy.getBySel('phone-number-control').type(defaultAccount.username);
      cy.getBySel('receive-recovery-code-button').should('be.enabled');

      for (let i = 0; i < 4; i += 1) {
        cy.getBySel(`confirmation-digit-${i}`).should('be.disabled');
      }
    });

    it.skip('[PF-66] Should enable "Proceed" button after confirmation code is typed in', () => {
      cy.intercept('POST', '/api/password-recovery', { statusCode: 204 });
    ...
    it.skip('[PF-68] Should not allow to proceed if confirmation code is invalid', () => {
      cy.intercept('POST', '/api/password-recovery', { statusCode: 204 });
    ...
    it.skip('[PF-69] Should proceed to the next step if confirmation code is valid', () => {
      interceptConfirmationCode();
      cy.getBySel('password-recovery-proceed-button').should('be.enabled');
      cy.getBySel('password-recovery-proceed-button').click();
      cy.getBySel('stepper-step-1').should('have.class', 'is-active');
    });
  });

  describe('New password validation', () => {
    it.skip('[PF-74] Should highlight length error if password is too short', () => {
      interceptConfirmationCode();
      triggerError('1aA', 'error-length');
    });

    it.skip('[PF-75] Should highlight lowercase error if password does not include lowercase letter', () => {
      interceptConfirmationCode();
      triggerError('AAA11111', 'error-lowercase');
    });

    it.skip('[PF-76] Should highlight uppercase error if password does not include uppercase letter', () => {
      interceptConfirmationCode();
      triggerError('aaa11111', 'error-uppercase');
    });

    it.skip('[PF-77] Should highlight digits error if password does not include digits', () => {
      interceptConfirmationCode();
      triggerError('aaaAAAAA', 'error-digits');
    });

    it.skip('[PF-78] Should enable "Confirm password" input if password is valid', () => {
      interceptConfirmationCode();
      cy.getBySel('password-recovery-proceed-button').click();
    ...
    it.skip('[PF-79] Should display error if New and Confirm passwords missmatch', () => {
      interceptConfirmationCode();
      cy.getBySel('password-recovery-proceed-button').click();
    ...
    it.skip('[PF-80] Should enable "Change password" button if confirm password matches new password', () => {
      interceptConfirmationCode();
      cy.getBySel('password-recovery-proceed-button').click();
