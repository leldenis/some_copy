...
import { COURIER_FLEET_TYPES } from '@ui/shared/consts';
import { Observable, filter, map, tap } from 'rxjs';

@Injectable({ providedIn: 'root' })
export class CouriersFleetGuard {
  constructor(
  ...
  public canActivate(): Observable<boolean> {
    return this.store.select(getSelectedFleet).pipe(
      filter(Boolean),
      map(({ fleet_type }) => COURIER_FLEET_TYPES.has(fleet_type)),
      tap((canActivate) => {
        if (!canActivate) this.router.navigate(['/', CorePaths.WORKSPACE, CorePaths.GENERAL]);
      }),
    );
  }
}
