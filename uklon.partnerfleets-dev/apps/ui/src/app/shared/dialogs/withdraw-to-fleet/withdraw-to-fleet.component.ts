...
interface WithdrawToFleetDialogData {
  balanceChange: BalanceChangeDto;
  currency: Currency;
}

@Component({
  selector: 'upf-withdraw-to-fleet',
  standalone: true,
  imports: [TranslateModule, MatIconButton, MatIcon, MoneyPipe, MatButton],
  templateUrl: './withdraw-to-fleet.component.html',
  styleUrls: ['./withdraw-to-fleet.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class WithdrawToFleetComponent implements OnInit {
  public amount: number;
  public changes: DriverBalanceChangeDto[];

  private readonly dialogRef = inject(MatDialogRef<WithdrawToFleetComponent>);
  public readonly icons = inject(ICONS);
  public readonly data = inject<WithdrawToFleetDialogData>(MAT_DIALOG_DATA);
